...
function computeHash_ResetPassword(password, salt, fn) {
	// Bytesize
	...
	if (3 == arguments.length) {
		...
		crypto.randomBytes(len, function(err, salt) {
			...
			computeHash_ResetPassword(password, salt, fn);
		});
	}
}

function getUser_ResetPassword(email, fn) {
	dynamodb.getItem({
...
function updateUser_ResetPassword(email, password, salt, fn) {
	dynamodb.updateItem({
			TableName: config.DDB_TABLE,
			Key: {
				email: {
...
exports.handler = function(event, context) {
	...
	getUser_ResetPassword(email, function(err, correctToken) {
		if (err) {
			...
			computeHash_ResetPassword(newPassword, function(err, newSalt, newHash) {
				if (err) {
					context.fail('Error in computeHash: ' + err);
				} else {
					updateUser_ResetPassword(email, newHash, newSalt, function(err, data) {
						if (err) {
