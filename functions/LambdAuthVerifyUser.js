...
function getUser_VerifyUser(email, fn) {
	dynamodb.getItem({
...
function updateUser_VerifyUser(email, fn) {
	dynamodb.updateItem({
			TableName: config.DDB_TABLE,
			Key: {
				email: {
...
exports.handler = function(event, context) {
	...
	getUser_VerifyUser(email, function(err, verified, correctToken) {
		if (err) {
			...
			// User verified
			updateUser_VerifyUser(email, function(err, data) {
				if (err) {
